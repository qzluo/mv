...
void CModBusRtu::setComm(SerialComm *comm)
{
    serialComm = comm;
}

void CModBusRtu::setSlaveAddr(int slaveAddr)
{
...
int CModBusRtu::readRegisters(int reg_addr, int reg_num, unsigned short *dest)
{
    //生成写命令
    char inBuf[8] = {};
    inBuf[0] = slaveAddr & 0xFF;
    inBuf[1] = 3;
    inBuf[2] = HIGHBYTE(reg_addr);
    inBuf[3] = LOWBYTE(reg_addr);
    inBuf[4] = HIGHBYTE(reg_num);
    inBuf[5] = LOWBYTE(reg_num);
    ...
    //发送命令及读响应
    int ret = oneTransaction(inBuf, outBuf, sizeof(inBuf), 5 + 2*reg_num);
    if (ret < 0)
        return -1;

    //检查命令响应
    if (outBuf[0] != inBuf[0] && outBuf[1] != inBuf[1])
        return -1;

    if (outBuf[2] != 2*reg_num)
        return -1;
    ...
    for (int i = 0; i < reg_num; ++i) {
        dest[i] = BYTESTOSHORT(outBuf[3 + 2 * i],outBuf[4 + 2 * i]);
    }
...
int CModBusRtu::writeRegister(int reg_addr, int value)
{
    ...
    inBuf[0] = slaveAddr & 0xFF;
    inBuf[1] = 6;
    inBuf[2] = HIBYTE(reg_addr);
    inBuf[3] = LOWBYTE(reg_addr);
    inBuf[4] = HIBYTE(value);
    inBuf[5] = LOWBYTE(value);
    ...
    //发送命令及读响应
    int ret = oneTransaction(inBuf, outBuf, sizeof(inBuf), sizeof(outBuf));
    if (ret < 0)
        return -1;
...
int CModBusRtu::writeRegisters(int reg_addr, int reg_num, const unsigned short *data)
{
    ...
    for (int i = 0; i < reg_num; ++i) {
        inBuf[7 + 2 * i] = HIGHBYTE(data[i]);
        inBuf[8 + 2 * i] = LOWBYTE(data[i]);
    }
    ...
    //发送命令及读响应
    if (oneTransaction(inBuf, outBuf, 9 + 2*reg_num, sizeof(outBuf)) < 0)
        return -1;
...
int CModBusRtu::oneTransaction(char *inData, char *outData, qint64 inDataLen, qint64 outDataLen)
{
    if (!serialComm)
        return -1;
    ...
    //send read command
    int ret = 0;
    ret = serialComm->write(inData, inDataLen);
    if (ret != inDataLen)
        return -1;

    //read back data from peer
    return serialComm->read(outData, outDataLen);
}
